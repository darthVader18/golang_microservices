...
import (
	...
	"golang_microservices/src/api/domain/github"
	"golang_microservices/src/api/clients/restclient"
	"net/http"
	"errors"
	"os"
	"io/ioutil"
...
func TestCreateRepoNoError(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			StatusCode: http.StatusCreated,
			Body:       ioutil.NopCloser(strings.NewReader(`{"id": 123,"name": "golang_microservice_API","full_name": "darthVader18/golang_microservice_API"}`)),
		},
	})
	...
	assert.EqualValues(t, "golang_microservice_API", response.Name)
	assert.EqualValues(t, "darthVader18/golang_microservice_API", response.FullName)
}