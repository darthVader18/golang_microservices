...
import (
	...
	"os"
	"golang_microservices/src/api/domain/repositories"
	"github.com/stretchr/testify/assert"
	"net/http"
	"io/ioutil"
	"strings"
	"sync"
	"golang_microservices/src/api/utils/errors"
)
...
func TestCreateRepoNoError(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			...
			Body:       ioutil.NopCloser(strings.NewReader(`{"id": 123, "name": "testing", "owner": {"login": "darthVader18"}}`)),
		},
	})
	request := repositories.CreateRepoRequest{Name: "testing"}
	...
	assert.EqualValues(t, "darthVader18", result.Owner)
}
...
func TestCreateRepoConcurrentNoError(t *testing.T) {
	...
	assert.EqualValues(t, "darthVader18", result.Response.Owner)
}
